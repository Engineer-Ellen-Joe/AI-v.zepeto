# -*- coding: utf-8 -*-
"""
cepta_network_lowrank.py

- AutoTokenizer 로더
- 위치 정보: 절대 사인·코사인
- 임베딩 모듈: CEPTA 토큰 임베딩(CeptaEmbedding, use_index=True)
- 트랜스포머 스택: Pre-LN(RMSNorm) + [CEPTA 경로 → SSM-lite(저랭크) → 라우팅] × N + 최종 RMSNorm + LM Head
- AMP 및 CE 손실 학습 스텝

의존: perceptron_cepta.py (CeptaEmbedding, CeptaRouting, CeptaConfig, CeptaSSMLiteLowRank)
"""
...
from perceptron_cepta import CeptaEmbedding, CeptaRouting, CeptaConfig, CeptaSSMLiteLowRank
...
# ------------------------------------------------------------
# 경로기반 블록: Pre-LN → CEPTA 경로 → SSM-lite(저랭크) → 라우팅 → 잔차 → Pre-LN → MLP → 잔차
# ------------------------------------------------------------
class CeptaBlock(nn.Module):
    def __init__(self, D: int, P: int, alpha: int,
                 *, route_mode: Literal['softmax','topk'] = 'softmax', topk: int = 8,
                 ...
                 gate: Literal['hard','ste_band','ste_sigmoid'] = 'hard',
                 dtype_store: Literal['bf16','fp16','fp32'] = 'bf16',
                 ssm: Literal['none','lowrank'] = 'none', P_r: int = 64, ssm_residual: bool = True):
        super().__init__()
        self.rms1 = RMSNorm(D)
        ...
        # 시간축 혼합(입력: t=(F⊙U), 형상: (B,T,P))
        self.ssm = CeptaSSMLiteLowRank(P, P_r, residual=ssm_residual) if ssm == 'lowrank' else None
        # 경로 라우팅(공간 혼합)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h1 = self.rms1(x)
        U, Fhard, Y = self.embed(X_dense=self.to_P(h1))  # (B,T,P)
        t = (Fhard.float() * U.float())
        if self.ssm is not None:
            t, _ = self.ssm(t, Fhard)
        routed = self.route(t)
    ...
    def __init__(self, cfg: ModelCfg):
        ...
        self.blocks = nn.ModuleList([
            CeptaBlock(D=cfg.d_model, P=cfg.P, alpha=cfg.alpha,
                       ...
                       ssm=cfg.ssm, P_r=cfg.P_r, ssm_residual=cfg.ssm_residual)
...
if __name__ == "__main__":
    ...
    cfg = ModelCfg(vocab_size=len(tok), d_model=768, n_layers=6, P=256, alpha=4,
                   ...
                   ssm='lowrank', P_r=64, ssm_residual=True)
